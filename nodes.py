...
class MossTTSDGenerate:
    ...
    def INPUT_TYPES(s):
        ...
        return {
            "required": {
                "moss_model": ("MOSS_TTSD_MODEL", {"tooltip": "Loaded MOSS-TTSD model."}),
                "moss_codec": ("MOSS_AUDIO_CODEC", {"tooltip": "Loaded MOSS Audio Tokenizer/Codec."}),
                "text": ("STRING", {"multiline": True, "default": "[S1] Hello world.", "tooltip": "Text to generate speech from. Use [S1]-[S5] tags for specific speakers."}),
                "mode": (["generation", "voice_clone", "continuation", "voice_clone_and_continuation"], {"default": "voice_clone", "tooltip": "Generation mode. 'voice_clone' is recommended for reference audio."}),
                "audio_temperature": ("FLOAT", {"default": 1.1, "min": 0.1, "max": 2.0, "step": 0.1, "tooltip": "Audio sampling temperature. Higher = more random/emotional."}),
                "audio_top_p": ("FLOAT", {"default": 0.9, "min": 0.1, "max": 1.0, "step": 0.05, "tooltip": "Nucleus sampling probability for audio."}),
                "audio_top_k": ("INT", {"default": 50, "min": 1, "max": 200, "tooltip": "Top-K sampling for audio."}),
                "audio_repetition_penalty": ("FLOAT", {"default": 1.1, "min": 1.0, "max": 2.0, "step": 0.1, "tooltip": "Penalty for repeating audio tokens."}),
                "text_temperature": ("FLOAT", {"default": 1.1, "min": 0.1, "max": 2.0, "step": 0.1, "tooltip": "Temperature for text generation (if applicable)."}),
                "max_new_tokens": ("INT", {"default": 2000, "min": 100, "max": 10000, "tooltip": "Maximum number of tokens to generate."}),
                "text_normalize": ("BOOLEAN", {"default": True, "tooltip": "Normalize text (clean punctuation, merge tags) before generation."}),
                "sample_rate_normalize": ("BOOLEAN", {"default": True, "tooltip": "Resample all reference audios to the same sample rate."}),
            },
            "optional": optional,
        }
