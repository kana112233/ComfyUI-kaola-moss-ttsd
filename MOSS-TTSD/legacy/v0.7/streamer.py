...
class AudioIteratorStreamer(BaseStreamer):
    ...
    def __init__(
        ...
        self.duration_code_length = int(
            self.CHUNK_SIZE * spt.input_sample_rate // spt.encoder_downsample_rate
        )
        self.overlap_code_length = int(
            self.OVERLAP_SECONDS * spt.input_sample_rate // spt.encoder_downsample_rate
        )
        ...
        self.token_cache = torch.zeros(
            self.MAX_TOKEN_LENGTH, self.channels, dtype=torch.long, device=device
        )
    ...
    def decode(self, last_chunk=False):
        duration_to_decode = min(
            self.duration_code_length,
            self.token_cache_length - self.decoded_idx - self.channels + 1,
        )

        speech_ids = torch.full((duration_to_decode, self.channels), 0).to(self.device)
        for j in range(self.channels):
            speech_ids[..., j] = self.token_cache[
                self.decoded_idx + j : self.decoded_idx + j + duration_to_decode, j
            ]
            if j == 0:
        ...
        with torch.no_grad():
            chunk_codes = speech_ids.permute(1, 0).unsqueeze(
                1
            )  # Convert to SPT expected format
            decode_result = self.spt.inference_detokenize(
                chunk_codes, torch.tensor([duration_to_decode], device=self.device)
            )
    ...
    def put(self, value: torch.LongTensor):
        ...
        if (
            self.token_cache_length
            >= self.decoded_idx + self.duration_code_length + self.channels - 1
        ):
