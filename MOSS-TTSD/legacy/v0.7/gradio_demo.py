...
def load_examples_from_jsonl():
    ...
    jsonl_paths = [
        ...
        "examples/examples_single_reference.jsonl",
    ]
    ...
    print(
        f"Loaded {len(role_examples)} role examples and {len(single_examples)} single examples from {jsonl_paths}"
    )
...
def create_gradio_interface() -> gr.Blocks:
    with gr.Blocks(
        title="MOSS-TTSD🪐 Dialogue Generation", theme=gr.themes.Soft()
    ) as demo:
        ...
        with gr.Row():
            with gr.Column():
                ...
                status_info = gr.Textbox(
                    label="Status Information", lines=10, interactive=False
                )
        ...
        with gr.Row():
            with gr.Column():
                ...
                examples_desc_md = gr.Markdown(
                    "Click on examples below to auto-fill the form"
                )
        ...
        def update_language(lang):
            ...
            return (
                ...
                gr.Button(
                    texts["generate_audio"], variant="primary", size="lg"
                ),  # generate_btn
                gr.Audio(
                    label=texts["generated_audio"], type="filepath"
                ),  # output_audio
                gr.Textbox(
                    label=texts["status_info"], lines=10, interactive=False
                ),  # status_info
        ...
        language_selector.change(
            ...
            outputs=[
                ...
                role_examples_md,
                single_examples_md,
                role_examples.dataset,
                single_examples.dataset,
            ],
        )
