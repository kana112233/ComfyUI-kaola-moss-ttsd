...
def process_data(
    ...
    for idx, item in enumerate(items):
        ...
        if "file_path" in item and "full_transcript" in item:
            ...
            input_id, labels, total_length, audio_length = process_inputs(
                tokenizer,
                spt,
                SYSTEM_PROMPT,
                final_text,
            ...
            "reference_audio" in item
            and "reference_text" in item
            and "audio" in item
            and "text" in item
        ):
            ...
            if not os.path.exists(reference_audio_path):
                print(
                    f"Warning: Reference audio file not found: {reference_audio_path}, skipping item {idx}..."
                )
                continue

            if not os.path.exists(audio_path):
                print(
                    f"Warning: Audio file not found: {audio_path}, skipping item {idx}..."
                )
                continue
            ...
            try:
                ...
                print(
                    f"Warning: Failed to load audio files: {e}, skipping item {idx}..."
                )
                continue

        else:
            print(
                f"Warning: Item {idx} missing required fields for both supported formats, skipping..."
            )
            continue
        ...
        print(
            f"Processed item {idx + 1}/{len(items)}: input_ids shape {input_id.shape}, labels shape {labels.shape}, total_len={total_length}, audio_len={audio_length}"
        )
    ...
    print(
        f"Saved metadata (pointers, tokens_lengths, tims_lengths) to {output_meta_path}"
    )
    print(f"Total sequences processed: {len(all_data)}")
    print(
        f"Average total length: {np.mean(tokens_lengths):.1f}, Average audio length: {np.mean(tims_lengths):.1f}"
    )
...
if __name__ == "__main__":
    ...
    process_data(
        args.jsonl, args.model_path, args.output_dir, args.data_name, args.use_normalize
    )
